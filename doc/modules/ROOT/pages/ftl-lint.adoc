...
Use `--format json` for editor and tool integration. JSON diagnostics go to standard output, while invocation and I/O errors remain on standard error. The xref:json-diagnostics.adoc[JSON diagnostics protocol] defines the stable integration interface.
...
|`--format <FORMAT>`
|Select `human` or `json` output; defaults to `human`

|`--color <WHEN>`
|Select `auto`, `always`, or `never`; defaults to `auto`
